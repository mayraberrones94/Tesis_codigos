...
nn_model = Sequential()
nn_model.add(Dense(256, input_shape=(7 * 7 * 512,), activation="relu"))
nn_model.add(Dense(16, activation="relu"))
nn_model.add(Dense(len(config.CLASSES), activation="sigmoid"))

# compile the model
opt = SGD(lr=1e-3, momentum=0.9, decay=1e-3 / 25)
nn_model.compile(loss="binary_crossentropy", optimizer='sgd',
	metrics=["accuracy"])
...
predIdxs = nn_model.predict(x=testGen,
